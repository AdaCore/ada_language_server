...
export default class gnatproveTaskProvider implements vscode.TaskProvider<vscode.Task> {
    ...
    resolveTask(
        ...
        if (definition.taskKind == taskKinds.proveRegion) {
            const args = getGnatproveArgs([
                '-u',
                '${fileBasename}',
                '--limit-region=${fileBasename}:' +
        ...
        // Additional arguments for examine/proof of subprogram need to be evaluated in a Promise
            ...
            definition.taskKind == taskKinds.examineSubprogram ||
            ...
            return getSubprogramSymbol(vscode.window.activeTextEditor).then((Symbol) => {
                if (Symbol) {
                    ...
                    let args =
                        definition.taskKind == taskKinds.examineSubprogram ? ['--mode=flow'] : [];
                    args = getGnatproveArgs(
                        args.concat([
                            '-u',
                            '${fileBasename}',
                            '--limit-subp=${fileBasename}:' + subprogram_line,
                        ])
                    );
                    const shell = new vscode.ShellExecution('gnatprove', args);
                    return new vscode.Task(
...
const getTasks = (): vscode.Task[] => {
    ...
    // Prove the project
    const proveProject = makeTask(taskKinds.proveProject, getGnatproveArgs([]), 'Prove project');
