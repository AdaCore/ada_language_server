...
* In Ada and GPR files, `Shift+F12` now runs `Find All References`, which displays the results in VS Code's `References` view instead of the editor's peek widget, so that they can be navigated with `F4`/`Shift+F4` and run again later from the view's history. See [Navigating references](./doc/vscode-ug.md#navigating-references)
