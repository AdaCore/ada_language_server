...
def print_spec(file, node):
    ...
    print(name)
    file.write(
        io_spec.format(type=name, kind="Read", out="out ").encode(encoding="utf-8")
    )
...
def print_components(file, kind, node, no_components):
    ...
    for x in get_components(node):
        ...
        txt = write_format(tp, kind).format(
            key=get_key(name), kind=kind, name=name, type=tp
        )
...
def print_enums(file, kind, type, node):
    if type not in enum_as_string:
        ...
        for x in node.finditer(lal.EnumLiteralDecl):
            name = x.p_defining_name.token_start.text
            txt += io_string_enum_case[kind].format(name=name, key=get_key(name))
...
def print_body(file, node):
    ...
    unref = (
        ...
        if len(list(node.finditer(lal.NullRecordDef)))
        else ""
    )
    ...
    def print_decls_and_statements(kind):
        file.write(
            io_header.format(
                type=name, kind=kind, out="" if kind == "Write" else "out ", unref=unref
            ).encode(encoding="utf-8")
        )
        ...
        file.write(
            io_footer.format(
                type=node.p_defining_name.token_start.text, kind=kind
            ).encode(encoding="utf-8")
        )
...
def print_io():
    ...
    unit = ctx.get_from_file("liblsp_3_16/source/lsp-messages.ads")
    ads = open("liblsp_3_16/source/generated/lsp-message_io.ads", "wb")
    ads.write(spec_header.encode(encoding="utf-8"))
    adb = open("liblsp_3_16/source/generated/lsp-message_io.adb", "wb")
