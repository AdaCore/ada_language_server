...
They can be used to build and run your program (`ada: Build current project` task) or launch external tools such as GNAT SAS, GNATprove and a few others. Most of these tools are integrated using [SARIF](https://sarifweb.azurewebsites.net/), a standard interchange format that can be easily parsed and displayed using the third-party [SARIF Viewer extension](https://marketplace.visualstudio.com/items?itemName=MS-SarifVSCode.sarif-viewer). The SARIF Viewer extension also supports SARIF fixes as [VS Code Actions](https://code.visualstudio.com/docs/editing/refactoring#_code-actions-quick-fixes-and-refactorings), so tools that emit them will have their fixes available as quick fixes directly in the editor and the **Problems View**.
