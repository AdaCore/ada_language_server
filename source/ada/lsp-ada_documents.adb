...
package body LSP.Ada_Documents is
   ...
   begin
      ...
      Old_Lines := Old_Text.Split_Lines
        (Terminators     => LSP_New_Line_Function_Set,
         Keep_Terminator => True);
      if Natural (Span.last.character) = 0 then
         --  add empty line to represent the last line if Span.last points
         --  at the start of it, so we don't have any symbols of it and as
         --  a result don't have the line at all
         Old_Lines.Append (Empty_Virtual_String);
      end if;
      ...
      declare
         ...
         --  to calculate span
         Current_Line_Number : Line_Number := Span.last.line;
