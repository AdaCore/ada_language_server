...
package body LSP.Formatters.Fallback_Indenter is
   ...
   type Construct_Att_Key is
     (Ada_Abstract_Attribute,
      Ada_Access_Attribute,
      Ada_Aliased_Attribute,
      Ada_Array_Attribute,
      Ada_Assign_Attribute,
      Ada_Constant_Attribute,
      Ada_Delta_Attribute,
      Ada_Digits_Attribute,
      Ada_In_Attribute,
   ...
   subtype Token_Class_Declk is Token_Type range Tok_Entry .. Tok_Procedure;
   ...
   type Construct_Type is
     (Conditional, Type_Declaration, Function_Call, Aggregate);
   --  Type used to differentiate different constructs to refine indentation

   package Construct_Stack is new
     Generic_Stack (Construct_Type);
   use Construct_Stack;
   ...
   package Indent_Stack is new Generic_Stack (Indent_Type);
   use Indent_Stack;
   ...
   function UTF8_Next_Char (Str : String; Index : Positive) return Positive;
   ...
   function UTF8_Get_Char (Input : String) return Wide_Wide_Character;
   --  Return first character of UTF8_String

   function UTF8_Prev_Char (Str : String; Index : Natural) return Natural;
   ...
   begin
      for J in P .. Buffer'Last - 1 loop
         if Buffer (J) = Ada.Characters.Latin_1.LF then
            Next := J + 1;
            Success := True;
            return;
         end if;
   ...
   function UTF8_Get_Char (Input : String) return Wide_Wide_Character is
   ...
   function UTF8_Next_Char (Str : String; Index : Positive) return Positive is
   begin
   ...
   function UTF8_Prev_Char (Str : String; Index : Natural) return Natural is
   ...
   begin
      ...
      while Result in Str'Range
        and then Str (Result)
                 in Character'Val (16#80#) .. Character'Val (16#BF#)
      loop
         Result := Result - 1;
      end loop;
   ...
   function Get_Token (Str : String; Prev_Token : Token_Type) return Token_Type
   is
      S : String (Str'Range);
   begin
   ...
   is
      ...
      Indent_Decl        : constant Natural := Indent_Level;
      Indent_With        : constant Natural := Indent_Level + Indent_Continue;
      Indent_Use         : constant Natural := Indent_Continue * 2;
      Indent_When        : constant Natural := Indent_Level + Indent_Continue;
      Indent_Conditional : constant Natural := Indent_Continue;
      Indent_Record      : constant Natural := Indent_Level;
      ...
      procedure Finish_Aspect_Clause (P, Line : Natural; Done : out Boolean);
      ...
      function Look_For
        (Index : Natural; S : String; Need_Whitespace : Boolean := True)
         return Boolean;
      ...
      begin
         ...
         loop
            ...
            exit when
              Buffer (Index) /= ' '
              and then Buffer (Index) /= Ada.Characters.Latin_1.HT;
         ...
         if Top_Token.Profile_Start = 0 then
            ...
            Do_Indent
              (Prec,
               ...
               - Line_Start (Buffer, Top_Token.Profile_Start)
               + 1);
         end if;
      ...
      begin
         ...
         return
           ...
           or else (Token = Tok_Not
                    and then Prev_Token
                             in Tok_And | Tok_Or | Tok_Then | Tok_Else)
      ...
      begin
         ...
         then
         ...
         elsif Is_Continuation_Line (Token, Prev_Token, Prev_Prev_Token) then
            Do_Indent (Prec, Line_Count, Num_Spaces, Continuation => True);
           ...
           and then (Token in Tok_When | Tok_Vertical_Bar
      ...
      begin
         ...
         loop
            ...
            while Tmp < Buffer_Last and then Is_Blank (Buffer (Tmp)) loop
               if Buffer (Tmp) = Ada.Characters.Latin_1.LF then
      ...
      begin
         return
           (Is_Blank (Buffer (Index - 1)) or else not Need_Whitespace)
      ...
      procedure Finish_Aspect_Clause (P, Line : Natural; Done : out Boolean) is
      ...
      begin
         ...
         if Reserved = Tok_Is and then Aspect_Clause and then Num_Parens = 0
         then
         ...
         if not Right_Assignment then
            case Reserved is
               ...
               when Tok_Record =>
                  if Prev_Token /= Tok_Null or else Prev_Prev_Token = Tok_Is
                  then
         ...
         case Top_Token.Token is
            when Tok_Procedure | Tok_Function | Tok_Entry =>
            ...
            when Tok_Type =>
               if Reserved = Tok_Record then
                  ...
                  if Prev_Token = Tok_With
                    or (Prev_Prev_Token = Tok_With and Prev_Token = Tok_Null)
                  then
                     Top_Token.Attributes (Ada_Tagged_Attribute) := True;
                  end if;
         ...
         then
           ...
           and then ((Reserved = Tok_If and then Num_Parens = 0)
         ...
         then
            if not In_Generic
              and then (Top_Token.Token = Tok_With
            ...
            if Prev_Token = Tok_Access or else Prev_Token = Tok_Protected then
               ...
               if Top_Token.Token = Tok_Generic and then Prev_Token /= Tok_With
               then
         ...
         then
            ...
            elsif Top_Token.Token = Tok_Then and then Reserved = Tok_Elsif then
               Top_Token.Token := Tok_If;
            end if;

         elsif Reserved = Tok_With then
            if not In_Generic then
               if Top_Token.Token = Tok_With or else Top_Token.Token = Tok_Use
               then
            ...
            then
               ...
               then
                  Aspect_Clause := True;
                  Aspect_Clause_Sloc :=
                    (Line_Count, Current + 1 - Start_Of_Line + 1, Current + 1);
                  Do_Push := True;
         ...
         elsif Reserved = Tok_Use
           and then (Top_Token.Token = No_Token
         ...
         then
            ...
            elsif Top_Token.Token = Tok_If and then Reserved = Tok_Then then
               --  Notify that we're past the 'if' condition

               Top_Token.Token := Tok_Then;

            elsif Reserved = Tok_Loop then
               if Top_Token.Token = Tok_While or else Top_Token.Token = Tok_For
               then
            ...
            then
               ...
               if Reserved = Tok_When then
                ...
                and then (Prev_Prev_Token /= Tok_Use
                          or Prev_Token /= Tok_All)) --  use all type
      ...
      begin
         ...
         then
            ...
            if Top_Token.Token = Tok_Function
              and then Reserved /= Tok_New
              and then (Reserved = Tok_Abstract
                        or else Reserved = Tok_Separate
                        or else Start_Of_Line
                                /= Line_Start (Buffer, Top_Token.Sloc.Index))
            then
               Indent_Function_Return (Prec);
               Num_Spaces := Num_Spaces + Indent_Level;
            end if;
         ...
         elsif Reserved = Tok_Return and then In_Declaration = Subprogram_Decl
         then
            Indent_Function_Return (Prec);
         ...
         then
            ...
            then
               ...
               if Reserved /= Tok_When or else Top_Token.Token /= Tok_Select
               then
                  Num_Spaces := Num_Spaces - Indent_Level;
               end if;
      ...
      is
         ...
         begin
            ...
            if Local_Top_Token.Align_Colon = 0 or else Num_Parens > 1 then
               return;
            end if;
            ...
            if Num_Parens /= 0 then
               ...
               while First_Paren < P and then Buffer (First_Paren) /= '(' loop
         ...
         begin
            if Num_Parens > 0
              and then Get_Indentation.Prev_Prev_Token
            ...
            then
               ...
               Compute_Indentation
                 (Tok_Arrow, Prev_Token, Prev2_Token, P, L, Num_Spaces);
            else
               Do_Indent (P, L, Num_Spaces);
            end if;
            ...
            while J < P - 1 loop
               if Buffer (J) = '('
                 and then (Buffer (J - 1) /= ''' or else Buffer (J + 1) /= ''')
         ...
         begin
            ...
            if Prev_Tmp < Buffer'First or else Is_Blank (Buffer (Prev_Tmp))
            then
               Long := 2;
            ...
            if P < Buffer_Last and then not Is_Blank (Buffer (Next_Char (P)))
            then
               Long := Long + 1;
            end if;
         ...
         begin
            if P > Buffer_Last
              or else (Buffer (P) /= Ada.Characters.Latin_1.LF
                       and then Buffer (P) /= Ada.Characters.Latin_1.CR)
            then
               return;
            end if;

            while P < Buffer_Last
              and then (Buffer (P) = Ada.Characters.Latin_1.LF
                        or else Buffer (P) = Ada.Characters.Latin_1.CR)
            loop
               if Buffer (P) = Ada.Characters.Latin_1.LF then
                  New_Line (L);
                  Indent_Done := False;
               end if;
         ...
         begin
            ...
            if not Indent_Done
              ...
              and then Buffer (Start_Of_Line - 2) /= Ada.Characters.Latin_1.LF
            then
            ...
            loop
               ...
               loop
                  --  Skip blank lines
                  --  ??? need to handle Ada.Characters.Latin_1.CR as well, although we now
                  ...
                  if P /= Buffer_Last then
                     while Buffer (P) = ' '
                       or else Buffer (P) = Ada.Characters.Latin_1.HT
                     loop
                        P := P + 1;
         ...
         begin
            ...
            while P < Buffer'Last
              and then Buffer (P + 1) /= Ada.Characters.Latin_1.LF
            loop
               P := P + 1;
            end loop;
         ...
         begin
            ...
            if Num_Parens = 0 then
               if In_Declaration = Subprogram_Decl
                 and then (Top_Token = null
                           or else not Top_Token.Attributes
                                         (Ada_New_Attribute))
               then
                  Is_Parameter := True;
            ...
            if Indent_Done then
               ...
               if Prev2_Token = Tok_Comma or else Prev2_Token = Tok_Ampersand
               then
                  Do_Indent (P, L, Num_Spaces);
               else
         ...
         begin
            if (Local_Top_Token.Token = Tok_Colon
              ...
              and then (Local_Top_Token.Variable_Kind
                        in Parameter_Kind .. Discriminant_Kind
                        or else (Local_Top_Token.Is_In_Type_Definition
                                 and then not Local_Top_Token.Attributes
                                                (Ada_Record_Attribute)
                                 and then not Local_Top_Token
                                                .Type_Declaration))
            then
               if Local_Top_Token.Token = Tok_Identifier
                 and then not (Local_Top_Token.Is_In_Type_Definition
                               and then not Local_Top_Token.Attributes
                                              (Ada_Record_Attribute)
      ...
      begin
         ...
         loop
            ...
            if (End_Reached
                and then Buffer (Buffer_Last) /= Ada.Characters.Latin_1.LF)
              or else Terminated
            then
               return;
            end if;
            ...
            case Buffer (P) is
               ...
               when '"' =>
                  ...
                  begin
                     ...
                     loop
                        ...
                        --  the line, and the indentation of every line that
                        --  follows would be computed from a stale line.
               ...
               =>
                  ...
                  case Buffer (P) is
                     ...
                     when '/' | ':' =>
                        ...
                        if P < Buffer'Last and then Buffer (P + 1) = '=' then
                           if Buffer (P) = ':'
                     ...
                     when '=' =>
                        ...
                        if P + 1 <= Buffer'Last and then Buffer (P + 1) = '>'
                        then
                           Handle_Arrow;
                        else
                  ...
                  if (Num_Parens = 0
                      ...
                      or else Prev2_Token in Tok_Arrow | Tok_Then | Tok_Else)
               ...
               when ',' | ';' =>
                  ...
                  if Buffer (P) = ';' then
                     ...
                     if Aspect_Clause and then Num_Parens = 0 then
                     ...
                     if Local_Top_Token.Token = Tok_Colon then
                        ...
                        if In_Declaration /= No_Decl
                          or else (Local_Top_Token.Token = Tok_Task
                     ...
                     if In_Declaration = Subprogram_Decl
                       and then not (Top_Token /= null
                                     and then Top_Token.Attributes
                                                (Ada_New_Attribute))
                     then
                        Is_Parameter := True;
                     elsif In_Declaration = Type_Decl then
               ...
               when ''' =>
                  ...
                  then
                     ...
                     begin
                        ...
                        --  terminator without counting the line, and the
                        --  indentation of every line that follows was
                        --  computed from a stale line.
      ...
      loop
         ...
         if Token = Tok_Identifier then
            ...
            begin
               ...
               if not In_Generic and then Token = Tok_Generic then
                  In_Generic := True;
               elsif In_Generic
                 and then Prev_Token
               ...
               if Token = Tok_Body
                 ...
                 or else (Token = Tok_Is
                          and then not In_Generic
                          and then In_Declaration /= Expression_Function)
               then
                  In_Declaration := No_Decl;
